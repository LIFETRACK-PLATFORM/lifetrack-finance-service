pipeline {
  ...
  stages {
    ...
    stage("Docker Build") {
      steps {
        sh "docker buildx build --builder lifetrack-builder --provenance=false --sbom=false -t finance-service:latest --load ."
      }
    }
  }
  ...
  post {
    ...
    failure {
      echo "Pipeline FAILED - finance-service #${env.BUILD_NUMBER}"
